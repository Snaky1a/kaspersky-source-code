...
WORD xus_decodePgpMime( const NOTEHANDLE  h_NOTE, 
	...
	if (pt_parts->pt->puc_ctnt)	{
		...
		do	{
			...
			//	in an attachment, reäct accordingly.
			ui_ = NULL;
			if (pt_ctnt->pc_objNm)
...
errJump:
	...
	//	because the rich-text stream may be reüsed in the note's post-save (R5) 
...
--- parameters & return ------
pt_SPN: Optional. Address of rich-text span structure giving the coördinates of 
...
static DWORD ul_ExtractDlgInit( const HWND  H, 
	...
	//	on the dialog so we can reäct directly to a user's change in activity 
	//	choice
